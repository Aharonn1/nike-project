...
const Register = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    // const emailExists = users.map((user) => user.firstName); // הנחה שלכל אובייקט user יש שדה email
    // console.log("emailExists", users.map((user) => user.firstName) );

    // if (emailExists) {
    //     console.log("emailExists" , emailExists)
    //   setErrorMessage("משתמש עם כתובת אימייל זו כבר רשום במערכת.");
    //   return; // מנע המשך שליחת הטופס
    // }
    ...
    if (emailExists.length > 0) {
      console.log("משתמשים עם שם זה:", emailExists);
      setErrorMessage("שם משתמש זה כבר קיים במערכת.");
      return; // מנע המשך שליחת הטופס
    }

    try {
      const response = await axios.post(
        "http://localhost:4031/api/auth/register",
        {
          firstName,
          lastName,
