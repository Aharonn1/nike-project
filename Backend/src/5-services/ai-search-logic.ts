...
import { chatWithGemini, createEmbedding } from "../2-utils/ai-service.js";
...
function extractFilters(query: string) {
    ...
    if (foundColor) filters.color = foundColor === 'grey' ? 'gray' : foundColor;

    // 3. חילוץ קטגוריה (לפי טבלת categoryshoes שלך)
    if (lowerQuery.includes('men') || lowerQuery.includes('גברים')) filters.categoryId = 1; 
    if (lowerQuery.includes('women') || lowerQuery.includes('נשים')) filters.categoryId = 2;
    if (lowerQuery.includes('kids') || lowerQuery.includes('ילדים')) filters.categoryId = 3;

    return filters;
}

export async function searchAllShoes(userQuestion: string, history: any[] = []): Promise<{ answer: string, context: any }> {
    try {
        const filters = extractFilters(userQuestion);
        const userVector = await createEmbedding(userQuestion);
        const userVectorJsonString = JSON.stringify(userVector);

        let whereClauses = [];
        let params: any[] = [userVectorJsonString];
        ...
        const filteredProducts: any[] = await dal.execute(vectorSearchSql, params);

        let contextForAI: AIProductContext[] = [];
        if (filteredProducts.length > 0) {
            ...
            const availableSizes: any[] = await dal.execute(sizesSql);
        ...
        const finalResponse = await chatWithGemini(fullPrompt, history);
        
        return {
            answer: (finalResponse as any).answer || (finalResponse as any).text,
