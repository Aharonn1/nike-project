...
const shoesImagesFolder = "./src/1-assets/images/";
...
async function saveImage(image: UploadedFile): Promise<string> {
    ...
    const absolutePath = shoesImagesFolder + uniqueImageName;
...
async function deleteImage(existingImageName: string): Promise<void> {
    try {
        ...
        await fsPromises.unlink(shoesImagesFolder + existingImageName);
    }
    catch (err: any) {
